...
def rollingMeanAndStd(
    ...
    Kept cache-free: per-series caching is handled via cache.getZStatsForSeries
    using deterministic on-disk keys (engine build + kline digest).
    """
    return core.rollingMeanAndStd(series, window)

# Legacy alias (robust median/IQR replaced by mean/std)
rollingMedianAndIqr = rollingMeanAndStd
...
def zscoreSeries(
    ...
    mean, std = cache.getZStatsForSeries(
        ...
        str(seriesId),
        winBars,
        lambda: rollingMeanAndStd(arr, winBars),
    )
...
def _spacingLegZ(
    ...
    winDays = float(overrides[f'SPACING_WIN_DAYS{suffix}'])
    winBars = max(int(round(winDays * bars)), 1)
    seriesId = f"s{suffix.strip('_')}"
    z, valid = zscoreSeries(ctx, leg, winBars, seriesId)
    return z, valid
...
def _energyStats(
    ...
    z, valid = zscoreSeries(ctx, csum, winBars, f"e{leg}")
    maskTrend = (np.abs(trendCode) == 1) & valid
    return z, maskTrend


def energyCsum(ctx, trendCode: np.ndarray, leg: str) -> np.ndarray:
    return cache.getSeriesArray(
        ctx,
        f"e{leg}",
        lambda: core.energyCsum(ctx, trendCode, leg),
    )
...
def _energyMask(
    ...
    model = str(overrides['MICRO_NRG_MODEL']).strip().lower()
    if model in ('none', ''):
        zeros = np.zeros_like(allowReg, dtype=float)
        return zeros, np.copy(allowReg), zeros

    winDays = float(overrides['MICRO_NRG_WIN_DAYS'])
    winBars = max(int(round(winDays * bars)), 1)
    energy12 = energyCsum(ctx, trendCode, '12')
    energy23 = energyCsum(ctx, trendCode, '23')
    energyZ12, energyReady12 = _energyStats(
        ctx, energy12, trendCode, '12', winBars
    )
    energyZ23, energyReady23 = _energyStats(
        ctx, energy23, trendCode, '23', winBars
    )

    if model == 'sum':
        thresh12 = float(overrides['MICRO_NRG_MIN_12'])
        thresh23 = float(overrides['MICRO_NRG_MIN_23'])
        energyReady = allowReg & energyReady12 & energyReady23
        energyMask = (
            energyReady
            & (energyZ12 >= thresh12)
            & (energyZ23 >= thresh23)
        )
        energyMoment = np.where(energyReady, np.minimum(energy12, energy23), 0.0)
        energyZ = np.where(energyReady, np.minimum(energyZ12, energyZ23), 0.0)
        return energyZ, energyMask, energyMoment

    thresh23 = float(overrides['MICRO_NRG_MIN_23'])
    energyReady = allowReg & energyReady23
    energyMask = energyReady & (energyZ23 >= thresh23)
    energyMoment = np.where(energyReady, energy23, 0.0)
    energyZ = np.where(energyReady, energyZ23, 0.0)
    return energyZ, energyMask, energyMoment
...
def spacingState(
    ...
    bars = bars_per_day(ctx)
    z12, ready12 = _spacingLegZ(
        ctx,
        s12,
        overrides,
        suffix="_12",
    ...
    thresh12 = float(overrides['SPACING_Z_MIN_12'])
    thresh23 = float(overrides['SPACING_Z_MIN_23'])

    spacingReady = allowReg & ready12 & ready23
    spacingMask = spacingReady & (z12 >= thresh12) & (z23 >= thresh23)
    spacingZ = np.where(spacingReady, np.minimum(z12, z23), 0.0)

    energyZ, energyMask, energyMoment = _energyMask(
        ctx, trendCode, allowReg, overrides, bars
    )

    finalMask = spacingMask & energyMask
