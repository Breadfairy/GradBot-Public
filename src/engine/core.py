...
def macroDynCarry(
    ...
    """Carry macro dyn peak across full+half micro trend families."""
    dyn = np.asarray(macroDyn, dtype=float)
    trend = np.asarray(trendCode, dtype=int)
    n = min(int(dyn.size), int(trend.size))
    out = np.zeros(n, dtype=float)
    if n == 0:
        return out

    family = np.zeros(n, dtype=int)
    family[(trend[:n] == 1) | (trend[:n] == 2)] = 1
    family[(trend[:n] == -1) | (trend[:n] == -2)] = -1
...
def macroDynFromMas(
    ...
    # Linear damping as EMA1-EMA3 spread collapses.
    mult = ratio
    ...
    target = np.sign(spread13) * dynMag
    maxStep = max((pctMaxVal - max(pctMinVal, 0.0)) * 0.25, 0.0)
    if maxStep <= 0.0 or target.size <= 1:
        return target
    ...
    for i in range(1, target.size):
        ...
        sameSide = (
            prev != 0.0
            and cur != 0.0
            and np.sign(prev) == np.sign(cur)
        )
        if sameSide:
            step = np.clip(cur - prev, -maxStep, maxStep)
            out[i] = prev + step
        else:
            out[i] = cur
    return out
